...
class listener implements EventSubscriberInterface
{
	...
	public function __construct(\phpbb\config\config $config, \phpbb\template\template $template, \phpbb\user $user, \phpbb\db\driver\driver_interface $db, $request, $root_path, $phpEx, $table_prefix, $header_images_table)
	{
		$this->config = $config;
	...
	if (isset($this->config['chi_enable']))
	{
		if ($this->config['chi_enable'] && ($this->config['chi_enable_guests'] && empty($this->user->data['is_registered']) || !empty($this->user->data['is_registered'])))
		{
			...
			{
				$site_logo = '<img src="' . $this->root_path . 'ext/dmzx/chl/images/logos/' . $site_logo_custom . '" alt="" title="" />';
			}
			if (!empty($header_background_custom))
			{
				$site_background_logo = $this->root_path . 'ext/dmzx/chl/images/backgrounds/' . $header_background_custom;
			}
		}
	}
